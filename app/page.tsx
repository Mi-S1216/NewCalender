...
const PRIORITY_COLORS: Record<number, string> = {
  5: '#FF3E80', 4: '#FF7E61', 3: '#FFC061', 2: '#CDFF61', 1: '#00FF79'
}
...
export default function Dashboard() {
  ...
  const addTodo = async (e: React.FormEvent) => {
    ...
    await supabase.from('todos').insert([{ user_id: DUMMY_USER_ID, title: todoTitle, target_month: targetMonth, due_date: todoDueDate || null, priority: todoPriority, is_completed: false }])
  ...
  const addCategory = async () => {
    ...
    await supabase.from('categories').insert([{ user_id: DUMMY_USER_ID, type: modalType, name: newCatName, color_code: newCatColor, is_default: false }])
    ...
    if (modalType === 'schedule') {
      ...
      if (scheduleMode === 'weekly' && scheduleEndDate && selectedDates.length === 1) {
        ...
        const { data: recData, error } = await supabase.from('recurring_schedules').insert([{
          user_id: DUMMY_USER_ID, start_date: targetDate, end_date: scheduleEndDate, day_of_week: new Date(targetDate).getDay()
        }]).select()
        ...
        while (curr <= end) {
          ...
          scheduleInserts.push({ user_id: DUMMY_USER_ID, category_id: categoryId, recurring_id: recurringId, title: scheduleTitle, start_time: startDateTime, end_time: endDateTime, is_all_day: isAllDay })
        ...
        const scheduleInserts = selectedDates.map(dateStr => {
          ...
          return { user_id: DUMMY_USER_ID, category_id: categoryId, title: scheduleTitle, start_time: startDateTime, end_time: endDateTime, is_all_day: isAllDay }
        })
        await supabase.from('schedules').insert(scheduleInserts)
      }
      ...
      if (isRecurring) {
        const ruleInserts = selectedDates.map(dateStr => {
          ...
          return { user_id: DUMMY_USER_ID, category_id: categoryId, type: modalType, amount: numAmount, day_of_month: dayOfMonth }
        })
        await supabase.from('finance_recurring_rules').insert(ruleInserts)
      }
      const txInserts = selectedDates.map(dateStr => ({
        user_id: DUMMY_USER_ID, category_id: categoryId, type: modalType, amount: numAmount, transaction_date: dateStr
      }))
  ...
  return (
    <main className="min-h-screen bg-gray-100 p-4 md:p-8 flex flex-col xl:flex-row gap-6 relative pb-32">
      <section className="flex-1 bg-white p-6 rounded-xl shadow-md border text-black">
        <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
          <div className="flex items-center gap-4">
            <button onClick={() => setCurrentDate(new Date(currentDate.getFullYear(), currentDate.getMonth() - 1, 1))} className="px-4 py-2 bg-gray-200 rounded hover:bg-gray-300">先月</button>
            <h2 className="text-2xl font-bold">
              {currentDate.getFullYear()}/{String(currentDate.getMonth() + 1).padStart(2, '0')}
            </h2>
            <button onClick={() => setCurrentDate(new Date(currentDate.getFullYear(), currentDate.getMonth() + 1, 1))} className="px-4 py-2 bg-gray-200 rounded hover:bg-gray-300">来月</button>
          </div>
          <div className="flex items-center gap-2">
            <button 
              onClick={() => { setIsMultiSelectMode(!isMultiSelectMode); setSelectedDates([]); }}
              className={`px-4 py-2 rounded font-bold text-sm transition-colors border-2 ${isMultiSelectMode ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-white text-gray-700 border-gray-300'}`}
            >
        ...
        <div className="grid grid-cols-7 gap-2">
          {getDaysInMonth().map((day, index) => {
            if (!day) return <div key={`empty-${index}`} className="min-h-[120px] bg-gray-50 rounded" />
            ...
            return (
              <div 
                ...
                className={`min-h-[120px] border rounded p-1 flex flex-col transition-all cursor-pointer overflow-hidden bg-white ${isSelected ? 'ring-4 ring-indigo-500 bg-indigo-50' : 'hover:shadow-md'}`}
              >
                  ...
                  {dailySchedules.map(sch => {
                    ...
                    return (
                      <div 
                        ...
                        className="text-xs px-1 rounded truncate cursor-pointer hover:opacity-80"
                ...
                <div className="mt-auto flex flex-col items-end text-[10px] sm:text-xs font-bold w-full">
      ...
      <section className="w-full xl:w-96 bg-white p-6 rounded-xl shadow-md border text-black">
      ...
      <div 
        className="fixed bottom-6 right-6 p-4 rounded-xl shadow-2xl border-2 font-bold text-lg z-40"
        style={{ backgroundColor: savingsBgColor, color: savingsTextColor, borderColor: savingsTextColor }}
      >
          ...
          <div className="bg-white p-6 rounded-lg w-[400px] text-black shadow-xl max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-xl font-bold text-indigo-700">
                {selectedDates.length === 1 ? selectedDates[0].replace(/-/g, '/') : `${selectedDates.length}日分の選択`}
              </h3>
                ...
                <div className="space-y-2">
                  {selectedDayTransactions.map(tx => (
                    <div key={tx.id} className="flex justify-between items-center text-sm bg-white p-2 rounded shadow-sm">
                      <div>
                        <span className={`font-bold mr-2 ${tx.type === 'income' ? 'text-blue-600' : 'text-red-600'}`}>
                ...
                {isAddingCategory ? (
                  <div className="border p-3 rounded mb-2 bg-gray-50">
                    ...
                    <div className="flex flex-wrap gap-2 mb-3">
                      {CUSTOM_COLORS.map(c => (
                        <div key={c.code} onClick={() => setNewCatColor(c.code)} className={`w-6 h-6 cursor-pointer border ${newCatColor === c.code ? 'ring-2 ring-black' : ''}`} style={{backgroundColor: c.code}} title={c.name} />
                      ))}
                    </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-lg w-80 text-black shadow-xl">
            ...
            <div className="flex flex-col gap-2">
              <button onClick={() => deleteSchedule('single')} className="w-full bg-red-100 text-red-700 border border-red-300 p-2 rounded font-bold hover:bg-red-200">
                この予定のみ削除
              </button>
              {selectedSchedule.recurring_id && (
                <button onClick={() => deleteSchedule('future')} className="w-full bg-red-600 text-white p-2 rounded font-bold hover:bg-red-700">
                  これ以降の定期予定も削除
                </button>
              )}
              <button onClick={() => setSelectedSchedule(null)} className="w-full bg-gray-200 p-2 rounded font-bold hover:bg-gray-300 mt-2">
                閉じる
              </button>
            </div>
          </div>
